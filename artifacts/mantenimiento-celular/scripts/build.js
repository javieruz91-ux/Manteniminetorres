...
async function findAvailableMetroPort() {
  const requestedPorts = [
    parsePort(process.env.METRO_PORT),
    parsePort(process.env.PORT),
  ].filter((port, index, ports) => port && ports.indexOf(port) === index);
  const candidates = [...requestedPorts];

  for (let port = 8081; port <= 8181; port++) {
  ...
  throw new Error('Could not find an available port for Metro');
}
...
async function startMetro(expoPublicDomain, expoPublicReplId) {
  ...
  console.log(`Selected Metro port: ${metroPort}`);
